...
function ph2o=ph2ocalc(coeffs,temp,salin)
    ph2o=1013.25*exp(coeffs.D0+coeffs.D1*(100./(temp+273.15))+ ...
...
function doxy=doxycalc(bphase,rphase,pres,temp,psal, coeffs)
    ...
    oxy=molardoxycalc(dphase,pres,temp, coeffs).*salcorrcalc(psal,temp, coeffs).*prescorrcalc(pres,temp, coeffs);
    doxy=oxy./rho';
end
